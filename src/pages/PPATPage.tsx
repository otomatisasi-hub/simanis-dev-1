...
import { WorksheetDetailView } from "@/components/ppat/WorksheetDetailView"
import { WorkflowDetailDialog } from "@/components/ppat/WorkflowDetailDialog"
import { PPATDetailView } from "@/components/ppat/PPATDetailView"
import { getWorkflowTemplate } from "@/data/ppatWorkflows"
import { FinanceDetailDialog } from "@/components/ppat/FinanceDetailDialog"
...
export function PPATPage() {
  const { currentModule } = useModule() // untuk halaman ini, seharusnya "ppat"
  ...
  const fetchData = async () => {
    try {
      ...
      // 3) Ambil requirement khusus ppat sebagai sumber filter
      ...
      // 4) Clients + services (nested); filter services per client dengan isPPATCombo
      ...
      const enrichedClients = (clientsData ?? [])
        .map((client) => {
          const onlyPPATServices = (client.services ?? [])
            .filter((s: any) => isPPATCombo(s))
            .map((s: any) => {
          ...
          return { ...client, services: onlyPPATServices }
        })
      ...
      // 5) Services list; filter dengan isPPATCombo dan enrich mandatory_documents
  ...
  const handleViewDocumentChecklist = (service: any) => {
    navigate(`/services/ppat/document-checklist/${service.id}`)
  }
  ...
  return (
    <div className="min-h-screen bg-muted/30">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="mb-6">
          <GlobalFilters menu="ppat" onFiltersChange={handleFiltersChange} />
        </div>
        ...
        >
            ...
            <Card>
              <CardHeader>
                <div className="flex justify-between items-center">
                  <CardTitle>Daftar Klien PPAT</CardTitle>
          ...
          <TabsContent value="lembar-kerja" className="space-y-4">
            <Card>
              <CardHeader>
                <CardTitle>Lembar Kerja PPAT</CardTitle>
              </CardHeader>
              <CardContent>
                {loading ? (
                  <div className="flex justify-center py-8">
                  ...
                  <div className="text-center py-8 text-muted-foreground">
                    Belum ada data lembar kerja ppat
                  </div>
                ) : (
                  <>
                    <Table>
          ...
          <TabsContent value="keuangan" className="space-y-4">
            <Card>
              <CardHeader>
                <div className="flex justify-between items-center">
                  <CardTitle>Keuangan PPAT</CardTitle>
                </div>
              </CardHeader>
              <CardContent>
                {loading ? (
