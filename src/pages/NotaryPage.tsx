...
import { WorksheetDetailView } from "@/components/notary/WorkSheetDetailView"
import { WorkflowDetailDialog } from "@/components/notary/WorkflowDetailDialog"
import { NotaryDetailView } from "@/components/notary/NotaryDetailView"
import { getWorkflowTemplate } from "@/data/notaryWorkflows"
import { FinanceDetailDialog } from "@/components/notary/FinanceDetailDialog"
...
export function NotaryPage() {
  const { currentModule } = useModule() // untuk halaman ini, seharusnya "notaris"
  ...
  const fetchData = async () => {
    try {
      ...
      // 3) Ambil requirement khusus notaris sebagai sumber filter
      ...
      // 4) Clients + services (nested); filter services per client dengan isNotarisCombo
      ...
      const enrichedClients = (clientsData ?? [])
        .map((client) => {
          const onlyNotaryServices = (client.services ?? [])
            .filter((s: any) => isNotarisCombo(s))
            .map((s: any) => {
          ...
          return { ...client, services: onlyNotaryServices }
        })
      ...
      // 5) Services list; filter dengan isNotarisCombo dan enrich mandatory_documents
      ...
      let financeQuery = supabase
        ...
        `)
        // hanya layanan Notaris
        .eq("services.menu_layanan", "notaris")
  ...
  const handleViewDocumentChecklist = (service: any) => {
    navigate(`/services/notaris/document-checklist/${service.id}`)
  }
  ...
  return (
    <div className="min-h-screen bg-muted/30">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="mb-6">
          <GlobalFilters menu="notaris" onFiltersChange={handleFiltersChange} />
        </div>
            ...
            <Card>
              ...
              <CardContent>
                {loading ? (
                  ...
                  <div className="text-center py-8 text-muted-foreground">
                    Belum ada data lembar kerja notaris
                  </div>
                ) : (
                  <>
                    <Table>
            ...
            <Card>
              ...
              <CardContent>
                {loading ? (
                  ...
                  <div className="text-center py-8 text-muted-foreground">
                    Belum ada data keuangan Notaris
                  </div>
                ) : (
                  <>
                    <Table>
